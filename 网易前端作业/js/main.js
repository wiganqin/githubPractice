...
*/
    ...
    */
	...
	/*
      轮播图模块
    */
    var Slideshow_module = (function(){
        var Slideshow_images = document.querySelectorAll('.Slideshow-item img');
        var Slideshow_area = document.querySelector('.Slideshow-area');
        var Slideshow_indicator_area = document.querySelector('.Slideshow-indicators');
        var Slideshow_indicators = Slideshow_indicator_area.querySelectorAll('li');
        var len = Slideshow_indicators.length; 
        var initial_class_of_indicator = Slideshow_indicators[0].className; 
        var Slideshow_state = true; //记录轮播图是否处于自动播放的状态

        //初始化当前处于active状态的item与indicator,这里指定为第一个
        Slideshow_indicators[0].className = initial_class_of_indicator + " active";
        Slideshow_images[0].style.opacity = '1';
     
        var Slideshow = function(targetIndex,currentIndex){
            //改变指示圆点的样式
            Slideshow_indicators[currentIndex].className = initial_class_of_indicator;
            Slideshow_indicators[targetIndex].className = initial_class_of_indicator + " active";
            //改变对应图片的样式，改变透明度，达到淡入淡出的效果
            Slideshow_images[currentIndex].style.opacity = '0';
            Slideshow_images[targetIndex].style.opacity = '1';
        };

        var autoSlideshow = function(){
            var currentIndex = getCurrentIndex(Slideshow_indicators),
                targetIndex;
            ...
            Slideshow(targetIndex,currentIndex);
        };

        var beginAutoSlideshow = setInterval(autoSlideshow,5000); //每隔500ms自动切换

        //用户手动切换，点击小圆点切换到对应的图片
        Slideshow_indicator_area.addEventListener('click',function(e){
        ...
        Slideshow_area.onmouseenter = function(){
            if(Slideshow_state){
                clearInterval(beginAutoSlideshow);
                Slideshow_state = false;   
            }
        };

        Slideshow_area.onmouseleave = function(){
            if(!Slideshow_state){
                beginAutoSlideshow = setInterval(autoSlideshow,5000);
                Slideshow_state = true;
            }
        };
    ...
    var getCourseLists = function(pageNo,psize,type,startPageIndex,currentPageIndex){
        ...
        var ajaxOnsuccess = function(xhr){
            ...
            for(i=0; i<lenOfCourseItems; i++){
                segment += 
                '<li class="course-item" '+'data-id="'+courseLists.list[i]['id']
				+'" data-name="'+courseLists.list[i]['name']
				+'" data-provider="'+courseLists.list[i]['provider']
				+'" data-learnercount="'+courseLists.list[i]['learnerCount']
				+'" data-categoryname="'+courseLists.list[i]['categoryName']
				+'" data-middlephotourl="'+courseLists.list[i]['middlePhotoUrl']
				+'" data-description="'+courseLists.list[i]['description']
								
				+'"><img src=' + courseLists.list[i]['bigPhotoUrl'] + ' alt="">'
			...
			//课程详情悬浮层
			course_display_area.onmouseover = function(event){
		          courseMsEnterHandler(event);
		         //console.log(courseLists.list[0]['id']);
	        }
			var courseMsEnterHandler = function(event){
				...
				//divX = target.offsetLeft + target.clientWidth;
				//divY = target.offsetTop; 
				
				var dataset = getDataset(target);
				var data={};
				...
				if(data.description!==undefined){
					...
					coursedetail += '<div  id="coursefloat"><div class="coursefloat-img"><img src="'
					+ data.middlephotourl
					+ '" alt="detail" ></div><div class="coursefloat-title"><h5> '
					+ data.name 
					+ '</h5><div class="learning-person-area"><p><span class="learning-person">'
					+ data.learnercount
					+ '</span>人在学</p></div><p class="coursefloat-p">发布者：'
					+ data.provider
					+ '</p><p class="coursefloat-p">分类：'
					+ data.categoryname
					+ '</p></div><div class="coursefloat-desc"><p>'
					+ data.description
					+ '</p></div></div>';
					
					var coursetest = document.getElementById("footer-last");
				    coursetest.innerHTML = coursedetail;
					// console.log(courseLists.list[0]);
					
					var crsDetailNode = document.getElementById("coursefloat");
					crsDetailNode.style.left = (divX + 30) + "px";
		            crsDetailNode.style.top = (divY+20) + "px";
					
				}
				// console.log(courseLists.list[1]);
			}
			
			
			//取消悬浮层
			course_display_area.onmouseout = function(event){
				courseMsLeaveHandler(event);
			}
			var courseMsLeaveHandler = function(event){
				var coursetest = document.getElementById("footer-last");
				coursetest.innerHTML = "";
			}
